...
var s_account="subwaysubwaykids"
...
s.linkInternalFilters = "javascript:,subwaykids.com"
...
function s_doPlugins(s) {
	...
	s.campaign=s.getQueryParam('rdr');
		
		s.campaign=s.getValOnce(s.campaign,'s_campiagn',0);
	
	s.prop18=s.eVar20=s.getDaysSinceLastVisit('s_lv');
	
	s.prop19=s.eVar21=s.getNewRepeat();
	
	s.prop11=s.eVar11=s.getTimeParting('h','-5','2008'); // Set hour 
	s.prop12=s.eVar12=s.getTimeParting('d','-5','2008'); // Set day
	s.prop13=s.eVar13=s.getTimeParting('w','-5','2008'); // Set Weekend / Weekday
...
var s_location = "buzz";
	...
	if(s_location.indexOf("buzz_me") != -1 || s_location.indexOf("spread_buzz") != -1 || s_location.indexOf("unsubscribe") != -1){
		s.setupFormAnalysis()
	}

if(s.pageName)s.eVar2=s.pageName
else s.eVar2=document.URL
...
if(s.prop9)s.eVar9=s.prop9
	...
	s.eVar14=s.getQueryParam('icid');
		...
		s.eVar14=s.getValOnce(s.eVar14,'s_var14',0);


var camp =s.getAndPersistValue(s.campaign,'s_cp_persist',0);
   
   if(s.campaign)
    s.prop10=s.campaign+" -- "  + s.pageName;
    
    else if(camp)
     s.prop10=s.pageName;

var icamp =s.getAndPersistValue(s.eVar14,'s_ev14_persist',0);
   
   if(s.eVar14)
    s.prop21=s.eVar14+" -- "  + s.pageName;
...
+"!=f5) return '';else return cval_s;");
 ...
 /*
	...
	// Only target forms on buzz_me, spread_buzz, and unsubsbribe
	
	//var s_location = document.location.toString().toLowerCase();
...
s.visitorNamespace="subwayfranchiseeadvertising"

s.trackingServer="tracking.subwayfreshbuzz.com"
